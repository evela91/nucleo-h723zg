...
int main(void)
{
  ...
  HAL_DMA_Start(&hdma_uart9_rx,(uint32_t) &huart9.Mask, (uint32_t)uartRxBuffer, RX_BUFFER_LENGTH);

  //__HAL_DMA_ENABLE(&hdma_uart9_tx);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  //HAL_UART_Transmit_DMA(&huart9 , data, sizeof (data));
